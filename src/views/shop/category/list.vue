<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" stripe style="width: 100%">
      <el-table-column prop="catalogId" label="类别ID" />
      <el-table-column prop="catalogName" label="类别名" />
      <el-table-column prop="createTime" label="创建时间" />
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="showUpdate(scope.row)">修改</el-button>
          <el-button type="danger" size="mini" @click="removeDataById(scope.row.catalogId)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="分类添加" :visible.sync="dialogFormVisible">
      <el-form :model="category">
        <el-form-item label="分类名字" :label-width="formLabelWidth">
          <el-input v-model="category.catalogName" ></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      add() {
        ...
        categoryApi.save(this.category).then(response => {
          this.category = {}
          this.fetchData()
        })
      },
      ...
      fetchData() {
        // 异步获取远程数据（ajax）
        categoryApi.getPageList(this.page, this.limit).then(response => {
